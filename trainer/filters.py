...
#lookup_expr -> icontains, startswith , endswith
...
class TrainerFilter(django_filters.FilterSet):
    ...
    class Meta:
        model = Trainer
        fields = ['first_name', 'last_name', 'course',  'start_date_gte', 'start_date_lte', 'end_date_gte', 'end_date_lte']

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.filters['first_name'].field.widget.attrs.update(
            {'class': 'form-control', 'placeholder': 'Plase enter first name'})
        self.filters['last_name'].field.widget.attrs.update(
            {'class': 'form-control', 'placeholder': 'Plase enter last name'})
        self.filters['course'].field.widget.attrs.update(
            {'class': 'form-control', 'placeholder': 'Please enter course'})